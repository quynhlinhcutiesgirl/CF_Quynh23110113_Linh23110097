...
def get_true_range(
    ...
    true_range = pd.concat(
        [
            prices_high - prices_low.astype(float),
            (prices_high - prev_close).abs().astype(float),
            (prices_low - prev_close).abs().astype(float),
        ],
        axis=1,
    )
    true_range = true_range.max(axis=1)
    true_range.name = "True Range"
    return true_range
...
def get_keltner_channels(
    ...
    """
    if window <= 0 or atr_window <= 0:
        raise ValueError("window and atr_window must be > 0")

    middle_line = get_exponential_moving_average(prices_close, window)
    middle_line = middle_line.astype(float).rename("Middle Line")
...
def get_bollinger_bands(prices: pd.Series, window: int, num_std_dev: float = 2.0, ddof: int = 0) -> pd.DataFrame:
    """
