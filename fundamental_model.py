...
Original file is located at
    https://colab.research.google.com/drive/1EFZxfSTmOPwriDn8RUVNXB_cCnpg_EJN
"""
...
def collect_financial_statements(
    ...
    # Drop known non-numeric metadata rows before attempting type conversion
    non_numeric_metadata_labels = ['reportedCurrency', 'filingDate', 'acceptedDate']
    # If the index is a MultiIndex, drop by level 1 (the metric name)
    if isinstance(financial_statement_total.index, pd.MultiIndex):
        financial_statement_total = financial_statement_total.drop(
            labels=non_numeric_metadata_labels, level=1, errors='ignore'
        )
    # If it's a simple Index, drop directly by label
    else:
        financial_statement_total = financial_statement_total.drop(
            labels=non_numeric_metadata_labels, errors='ignore'
        )
...
def export_fundamental_to_csv(
    ...
    enforce_source: str | None = "FinancialModelingPrep",
):
    if enforce_source == "FinancialModelingPrep" and not api_key:
      raise ValueError(
            "FMP API key required when enforce_source='FinancialModelingPrep'. "
            "Set api_key='YOUR_FMP_KEY' or set enforce_source=None/ 'YahooFinance'."
        )

    fund_total, fund_stats, no_data = collect_financial_statements(
        tickers=ticker,
        statement=statement,
        api_key=api_key,
        start_date=start_date,
    ...
    if ticker in no_data or fund_total.empty:
        print(f" Không có dữ liệu fundamental cho ticker: {ticker}")
        return None

    # Tên file
    if filename is None:
        freq = "Q" if quarter else "Y"
        filename = f"{ticker}_{statement}_{freq}_{start_date}_{end_date}.csv"

    # Xuất CSV
    fund_total.to_csv(filename)
    print(f" Đã xuất fundamental → {filename}")

    return fund_total

# dùng hàm
export_fundamental_to_csv(
    ticker="MSFT",
